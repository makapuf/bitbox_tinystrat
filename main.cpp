...
void get_possible_targets(Unit &attacking)
{
    ...
    for (int i=0 ; i<MAX_UNITS ; i++) {
        ...
        if (u->player() == game_info.current_player) continue; // same player

        // unit within reach ?
        if (attacking.can_attack(*u)) {
            game_info.targets[game_info.nbtargets++]=u;
            message("possible target : \n");
            u->info();
        }

        // enough targets ?
        if (game_info.nbtargets == 8) break;
    }
}
...
void sprite_set_palette(object *o, int palette)
{
    o->b = (uintptr_t)&data_palettes_bin[1024*palette];
}

// make object opponent ?
void combat (Unit &attacking, Unit &attacked)
{
    ...
    // fixme faire un opponent object

    object *attacking_face = game_info.face;
    uint8_t attacking_terrain = attacking.terrain();
    uint8_t attacking_player = attacking.player();
    ...
    attacking_sprite->fr = attacking.type()*8;
    object *attacking_life = sprite3_new(data_bignum_16x24_spr, 34,52,5);
    attacking_life->fr = attacking.health();
    ...
    uint8_t attacked_terrain = attacked.terrain();
    uint8_t attacked_player = attacked.player();
    ...
    attacked_sprite->h *= 2;
    attacked_sprite->fr = attacked.type()*8 + 2; // facing left
    object *attacked_life = sprite3_new(data_bignum_16x24_spr, 340,52,5);
    attacked_life->fr = attacked.health();
